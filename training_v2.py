...
from tensorflow.python.keras import regularizers as R


#
#from layers import Input, Conv2D, Lambda, Flatten, merge, multiply, maximum, subtract, add, Dense, dot, Flatten,MaxPooling2D
...
convnet = M.Sequential()
convnet.add(L.Flatten(input_shape=(IMG_W, IMG_H)))
# convnet.add(keras.layers.BatchNormalization(axis=1, batch_input_shape=(IMG_W*IMG_H,)))
convnet.add(L.Dense(1, activation='relu', input_shape=(IMG_W*IMG_H,), kernel_initializer='random_uniform', bias_initializer=keras.initializers.Constant(value=0), kernel_regularizer=keras.regularizers.l2(0.1), bias_regularizer=keras.regularizers.l2(0.1)))


# ====== SIAMESE NETWORK ======
siamese_input = L.Input((2,IMG_W,IMG_H))
norm_input = siamese_input
# norm_input = keras.layers.BatchNormalization()(siamese_input)
left_input = L.Lambda(lambda x: x[:, 0, :, :], output_shape=(batch_sz,IMG_W,IMG_H),)(norm_input);
right_input = L.Lambda(lambda x: x[:, 1, :, :], output_shape=(batch_sz,IMG_W,IMG_H),)(norm_input);
encoded_l = convnet(left_input)
encoded_r = convnet(right_input)
# calculate the loss
diff = tf.subtract(encoded_l, encoded_r)
dist_sq = L.dot([diff, diff], axes=1, normalize=False)

#abs_diff_lambda = lambda x: K.abs(x[0]-x[1])
#abs_diff = L.merge([encoded_l, encoded_r], mode = abs_diff_lambda, output_shape=lambda x: x[0])
#dist_sq = L.dot([abs_diff, abs_diff], axes=1, normalize=False)
#tf.Print(dist_sq, [dist_sq], message="dist_sq:")
...
# ====== TRAINING ======
siamese_net = M.Model(outputs=dist_sq, inputs=siamese_input) ## OUTPUT IS HERE+ ========
